...
var /*remoteConfigURL = 'https://raw.githubusercontent.com/WeCodeEmail/wce-environment/main/nodejs/node_modules/wecodeemail/config.json',*/
	remoteConfigURL = loadJsonFile.sync('d:/wce-environment/nodejs/node_modules/wecodeemail/config.json'),
...
function startExisting(remoteConfig) {
	var projectDir = basePath + remoteConfig.data.path  + '/',
		gulpSettings = fs.existsSync(projectDir + 'gulpfile.js'),
		gruntSettings = fs.existsSync(projectDir + 'Gruntfile.js');
	...
	showMenu(remoteConfig.updateMenu, function(userSelection) {
		...
		remoteConfig.updateMenu.forEach(function(item) {
			...
			if(item.value === selectedMode) {
				...
				if (gruntSettings) {
					console.log(colors.red('Grunt settings found'));
					console.log(colors.green('Change settings to Gulp'));
					var projectDirTemp = basePath + remoteConfig.data.path + '_temp/',
						filesToAdd,
						filesToRemove;

					filesToRemove = item.ignore.map(function(value){
						return projectDir + value.matchValue;
					});

					del.sync(filesToRemove);

					fs.copySync(projectDir, projectDirTemp + item.copy_to + '_temp');

					del.sync(projectDir);

					fs.renameSync(remoteConfig.data.path + '_temp', remoteConfig.data.path);

					filesToAdd = remoteConfig.data.source.map(function(value){
						if (value.excludeIf == 'wce') {
							return value.matchValue;
						}
					});

					filesToAdd.forEach(function(item) {
						fs.copySync(item, projectDir);
					})

					del.sync(projectDir + item.copy_to);

					fs.renameSync(projectDir + item.copy_to + '_temp', projectDir + item.copy_to);
				} else if (gulpSettings) {
					console.log(colors.green('Gulp settings found'));
				} else {
					console.log(colors.red('Error! Grunt and Gulp files are missing!'));
				};
...
function cloneGitRepo() {
	...
	var cloneRepo = function() {
		...
		if(repoURL) {
			fs.copySync(repoURL, projectConfig.path);
			currentCloneIndex++;
			cloneRepo();
		} else {
